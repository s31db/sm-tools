...
class Pareto(Chart):
    ...
    def build(self):
        ...
        ax2.plot(
            self._values.keys(),
            [v["per_sum"] for v in self._pareto_items.values()],
            linewidth=1,
            color="red",
            marker="s",
        )
    ...
    def build_percent(self, percent, color, lw: float = 1, linestyle: str = "--"):
        ...
        plt.plot(
            [x_pos_percent, len(self._values.keys()) - self._margin],
            [percent, percent],
            linestyle=linestyle,
            lw=lw,
            color=color,
...
if __name__ == "__main__":
    analyse = {
        "Retard transporteur": 175,
        "Retard fournisseur": 36,
        "Problème qualité": 9,
        "Erreur d'étiquetage": 16,
        "Erreur de stock": 8,
        "Erreur de saisie": 11,
        "Compte client bloqué": 5,
        "Colis perdu": 132,
        "Client absent": 12,
    }
    print(
        Pareto()
        .values(analyse)
        .y_label("Nb d'événements")
        .build()
        .build_percent(30, "black")
        .build_percent(90, "pink", 2, linestyle="dotted")
        .show()
        .img64()[0]
    )
