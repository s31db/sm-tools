...
def father(node: dict, lvl: int) -> Tuple[str | None, str | None, str | None, str | None]:
    sup = 'super' + ('.super' * (lvl - 1))
    if sup in node:
        return node[sup], node[sup + '.status'], node[sup + '.name'] if sup + '.name' in node else None, \
        ...
        for i in range(1, 10):
            ...
            if f:
                ...
                if estimate is None:
                    estimate = 1
                ti = {}
                for tim in ['aggregatetimeoriginalestimate', 'aggregatetimeestimate', 'aggregatetimespent']:
                ...
                if f in ids:
                    ...
                    ids[f] = {'lvl': i, 'children': {child_key: ids[child_key]}, 'estimate': estimate,
                              'done': ids[child_key]['done'], 'status': s, 'name': n, 'type': typ}
                    for tim in ['aggregatetimeoriginalestimate', 'aggregatetimeestimate', 'aggregatetimespent']:
                        ids[f][tim] = ti[tim]
                child_key = f
            else:
...
def test_tree() -> Tuple[dict, dict]:
    example = {'us1': {'super': 'fe1', 'estimate': 2, 'status': 'To Do', 'super.status': 'In Progress'},
               'us2': {'super': 'fe1', 'estimate': 4, 'status': 'Done', 'super.status': 'In Progress'},
               'us3': {'super': 'fe2', 'super.super': 'fe1', 'estimate': 3, 'status': 'Done', 'super.status': 'Done',
                       'super.super.status': 'In Progress'},
               'us4': {'super': 'fe1', 'estimate': 1, 'status': 'Done', 'super.status': 'In Progress'},
               'us5': {'super': 'fe1', 'estimate': 0, 'status': 'Not Done', 'super.status': 'In Progress'},
               }
