...
def workload_analyse(
    ...
    now = datefile(date_file)
    ends = (
        f"workloads_{project}_{fmt_date_file(start_date)}_"
    ...
    for datas in datas_sm:
        ...
        if month not in works_project[_project][datas["author"]]:
            works_project[_project][datas["author"]][month] = {
                "time": None,
                "timeSpentSecond": 0,
                "tickets": [],
            }
        if datas["day"] not in works_project[_project][datas["author"]]:
            works_project[_project][datas["author"]][datas["day"]] = {
                "time": None,
                "timeSpentSecond": 0,
                "tickets": [],
            }

        works_project_global[_project]["timeSpentSecond"] += int(
            datas["timeSpentSecond"]
        )
        if datas["key"] in works_project_global[_project]["tickets"]:
            works_project_global[_project]["tickets"][datas["key"]] += int(
                datas["timeSpentSecond"]
            )
        else:
            works_project_global[_project]["tickets"][datas["key"]] = int(
        ...
        works_project[_project][datas["author"]][month]["timeSpentSecond"] += int(
            datas["timeSpentSecond"]
        )
        works_project[_project][datas["author"]][datas["day"]][
            "timeSpentSecond"
        ] += int(datas["timeSpentSecond"])
        ...
        works[datas["author"]][month]["projects"][_project]["timeSpentSecond"] += int(
            datas["timeSpentSecond"]
        )
        works[datas["author"]][datas["day"]]["projects"][_project][
            "timeSpentSecond"
        ...
        works[datas["author"]][datas["day"]]["projects"][_project]["tickets"].append(
            datas
        )

        works[datas["author"]][month]["timeSpentSecond"] += int(
            datas["timeSpentSecond"]
        )
        works[datas["author"]][datas["day"]]["timeSpentSecond"] += int(
            datas["timeSpentSecond"]
        )
    ...
    with open(
        data_conf["Common"]["path_data"] + "calc_projects_" + ends,
        "w",
        encoding="utf-8",
    ) as f:
    ...
    with open(
        data_conf["Common"]["path_data"] + "calc_personnes_" + ends,
        "w",
        encoding="utf-8",
    ) as f:
...
def analyse_workload(start_date: str, date_to: str, date_file: str | None = None):
    ...
    ends = (
        "workloads"
        + fmt_date_file(start_date)
        + "_"
        + fmt_date_file(date_to)
        + "_"
        + now.replace("-", "")
        + ".json"
    )
    ...
    with open(
        data_conf["Common"]["path_data"] + "teams.json", "r", encoding="utf-8"
    ) as fp:
        datas_sm = json.load(fp)
    with open(
        data_conf["Common"]["path_data"] + "calc_personnes_" + ends,
        ...
        if person not in workloads:
            ...
            if month in workloads[person]:
                if team not in sum_project:
                    sum_project[team] = {}
                for projects, projects_infos in workloads[person][month][
                    "projects"
                ].items():
                    print("\t", month, projects, projects_infos["time"])
                    for t in ["all", team]:
                        if projects in sum_project[t]:
                            sum_project[t][projects][
                                "timeSpentSecond"
            ...
            for d in ds:
                if d not in workloads[person]:
                    ...
                    print(
                        ...
                        "\t",
                        "pb works",
                        workloads[person][d]["time"],
                        " ".join(
                            [
                                key + " " + v["time"]
                                for key, v in workloads[person][d]["projects"].items()
                            ]
                        ),
                    )
                else:
                ...
                if "other_projects" in infos:
                    for other_project, other_project_infos in infos[
                        "other_projects"
                    ].items():
...
def analyse_workload_planned(
    ...
    ds = dates(
        ...
        weeks=0,
        now=limit_today,
        end_date=datefile(date_to) if not limit_today else None,
    )
    sum_project = {"all": {}}
    ...
    with open(
        f"{data_conf['Common']['path_data']}calc_personnes_workloads_{project}_{ends}",
        "r",
        encoding="utf-8",
    ) as f:
        workloads = json.load(f)
    ...
    with open(
        data_conf["Common"]["path_data"] + "plan" + ends, "r", encoding="utf-8"
    ) as f:
        planneds = json.load(f)

    with open(
        data_conf["Common"]["path_data"] + "teams" + ends, "r", encoding="utf-8"
    ) as f:
    ...
    for team, members in teams_members.items():
        for person, person_infos in members.items():
            ...
            if person_id not in members_used:
                ...
                person_name = " ".join(
                    [p[0].upper() + p[1:] for p in person.split("@")[0].split(".")]
                )
                if (
                    "dateToANSI" not in person_infos
                    or start_date <= person_infos["dateToANSI"]
                ):
                    ...
                    if person_id in workloads:
                        if d in workloads[person_id]:
                            ...
                            for key, v in workloads[person_id][d]["projects"].items():
                                d_person[person_id]["days"][d]["works"][key] = v[
                                    "timeSpentSecond"
                                ]
                            ...
                            for project, projects_infos in workloads[person_id][d][
                                "projects"
                            ].items():
                                if project in sum_project[month]:
                                    sum_project[month][project][
                                        "timeSpentSecond"
                                    ] += projects_infos["timeSpentSecond"]
                                else:
                                    sum_project[month][project] = {
                                        "timeSpentSecond": projects_infos[
                                            "timeSpentSecond"
                                        ]
                                    }

                                if project in sum_project["all"]:
                                    sum_project["all"][project][
                                        "timeSpentSecond"
                                    ] += projects_infos["timeSpentSecond"]
                                else:
                                    sum_project["all"][project] = {
                    ...
                    if person in planneds and d in planneds[person]:
                        ...
                        for project_key, projects_infos in planneds[person][d].items():
                            ...
                            if project in sum_project_planned[month]:
                                sum_project_planned[month][project][
                                    "timeSpentSecond"
                                ] += seconds
                            else:
                                sum_project_planned[month][project] = {
                                    "timeSpentSecond": seconds
                                }

                            if project in sum_project_planned["all"]:
                                sum_project_planned["all"][project][
                                    "timeSpentSecond"
                                ] += seconds
                            else:
                                sum_project_planned["all"][project] = {
            ...
            for project, project_info in projects.items():
                if "timeSpentSecond" in project_info:
                    print(
                        ...
                        d,
                        project,
                        time_days(project_info["timeSpentSecond"]),
                    )
...
def worklog_plan_html(
    ...
    workload(
        project=project,
        start_date=start_date,
        date_to=datefile(date_to) if not limit_today else None,
    )
    workload_analyse(
        project=project,
        start_date=start_date,
    ...
    for p_infos in d_person.values():
        yield (
            f"<details open><summary>{p_infos['name']} <button class='cp' "
...
def sprints(project: str, suffix: str = ""):
    ...
    print(
        data_conf["projects"][project]["path_data"]
        + now.replace("-", "")
        + project
        + "_"
        + "infos_sprints"
    ...
    for key, sprint in s.items():
        if sprint["state"] != "future":
            ...
            for ticket, details in datas_sm[sprint["start_date"]].items():
                if details["sprints"] is not None and key in [
                    sp.split("[id=")[-1].split(",rapidViewId=")[0]
    ...
    for key, sprint in sprints_info.items():
        ...
        realized = sum(
            [
                float(t["estimate"])
                for t in sprint["end"].values()
                if t["estimate"] is not None
                and t["status"] in ("Done", "Fini", "Closed")
            ]
        )
        total = sum(
            [
        ...
        if da <= now:
            for ticket, value in datas_sm[da].items():
                if ticket not in tickets and value["status"] in ("Done", "Closed"):
                    ...
                    dates_end[da][ticket] = {"estimate": estimate, "date": da[:7]}
                    tickets.append(ticket)
