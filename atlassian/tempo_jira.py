...
class Tempo:
    ...
    def conn(self) -> "atlassian.tempo_jira.Tempo":
        self._tempo = client_v4.Tempo(
            base_url=self._url_server + "rest/tempo-planning/1",
            auth_token=self._token_auth,
        )
        return self
        ...
        for team, members in teams_datas.items():
            for member, values in members.items():
                if member not in members_used:
                    end_date_member = (
                        date_to if "dateToANSI" not in values else values["dateToANSI"]
                    )
                    if end_date_member >= start_date:
                        plan[member] = {}
                        members_used.append(member)
                        self._tempo._base_url = (
                            self._url_server + "rest/tempo-planning/1"
                        )
                        ...
                        for i in worklogs:
                            if "assignee" in i and not i["planItem"][
                                "summary"
                            ].startswith(self._tempo_exclude):
                                start = i["start"]
                                end = i["end"]
    ...
    def tempo_teams(self, start_date, teams) -> "atlassian.tempo_jira.Tempo":
        for team, members in teams.items():
            ...
            for team_info in teams_info:
                ...
                if (
                    "dateToANSI" not in team_info["membership"]
                    or not team_info["membership"]["dateToANSI"]
                    or team_info["membership"]["dateToANSI"] <= start_date
                ):
                    if (
                        "dateFromANSI" in team_info["membership"]
                        and team_info["membership"]["dateFromANSI"]
                    ):
                        member["dateFromANSI"] = team_info["membership"]["dateFromANSI"]
                    if (
                        "dateToANSI" in team_info["membership"]
                        and team_info["membership"]["dateToANSI"]
                    ):
